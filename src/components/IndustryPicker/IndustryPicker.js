...
import { Button } from '../../../components/hsReact/Button/index';
import { Popup, PopupFooter } from '../../../components/hsReact/Popup/index';
// import IndustryData from './IndustryData';
import './IndustryPicker.css';

export default class IndustryPicker extends React.Component {
    static defaultProps = {
        title: '',
        contWidth: 771,
        mask: true,
    ...
    componentWillReceiveProps(nexProps) {
        if (this.props.show !== nexProps.show) {
            this.setState({
                industryData: nexProps.data,
                level_1_Industry: '',
                level_2_Industry: '',
                level_3_Industry: '',
                ...
                arr_level_1_Industry: [],
                arr_level_2_Industry: [],
                arr_level_3_Industry: [],

                first_Level_Child_Data: [],
    ...
    getAllIndustry = () => {
        ...
        if (this.props.level === 3) {
            selectedIndustry.level_3 = { 'id': this.state.level_3_IndustryID, 'text': this.state.level_3_Industry };
        }
        this.props.onSure ? this.props.onSure(selectedIndustry) : null;
    }

    render() {
        const { title, contWidth, mask, model, show, level, onRequestClose, onSure, className, ...others } = this.props;
        const cls = classNames({
            'industryWrapper': true,
            'industryWrapper2': level !== 3,
            [className]: className
        });
        return (
            <div className="industryPickerWrapper"  {...others}>
                <Popup
                    title={title}
                    contWidth={contWidth}
                    mask={mask}
                    ...
                    <div className="industryPicker">
                        ...
                        <p className="seletedType">
                            ...
                            <span>
                                ...
                                {this.state.level_2_Industry}
                                {level === 3 && this.state.level_2_Industry ? `${this.state.level_2_Industry} > ` : ''}
                                {level === 3 && this.state.level_3_Industry}
                            </span>
                        </p>
                    </div>
                    <PopupFooter style={{ marginTop: '30px' }}>
